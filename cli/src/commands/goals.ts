...
export async function goalsCommand(options: GoalsOptions) {
...
async function listActiveGoal(activePath: string): Promise<void> {
  ...
  if (g.successCriteria?.length > 0) {
    ...
    g.successCriteria.forEach((c: string) => {
  ...
  if (g.scopeAlarms?.length > 0) {
    ...
    g.scopeAlarms.slice(-3).forEach((alarm: any) => {
...
async function showGoal(activePath: string, cwd: string, id: string): Promise<void> {
...
async function completeGoal(activePath: string, historyPath: string, cwd: string, id: string): Promise<void> {
  const active = await fs.readJson(activePath);
  ...
  // Update history
  let history = { version: '1.0.0', lastUpdated: '', goals: [] as any[] };
